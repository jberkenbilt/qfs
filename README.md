...
bump repo version to 2 and include a header with the minimum qfs version.

Versions of qfs prior to 0.3.0 would not work properly with SeaweedFS, an S3-compatible backends
that (erroneously) store S3 objects as files whose names are based too closely on the keys. If a
symlink target contained `/../` in its path, SeaweedFS would reject the key, even though this is
completely valid as an S3 object key. (As it happens, qfs wouldn't work with any S3-compatible
backend that required path-based access anyway, so no one would have been using qfs this way.)
Starting with qfs version 0.3.0, a `/` character that appears in a symlink *target* is "quoted"
using the `@` character. The old unquoted representation is still a valid representation of the
target, but qfs 0.3.0 and later will no longer write that format.
...
Before qfs 1.0, we will have some mechanism to store the minimum required qfs version that's safe to
use with a repository. Bumping the repo db version and including a minimum qfs version in the DB
would most mitigate it.

It is possible for running qfs ≥ 0.3.0 to lose data. Consider the following (rest of this section
mostly generated by Claude opus 5.5 but carefully vetted):

1. The repo has a link `x` pushed by 0.2.x with target `a/b`. Its S3 key is `x@l,T1,a/b`.
2. Locally, the link is replaced by a regular file with an older mtime `T0`. A 0.3.0 push calls
   `Remove` or `RemoveBatch`. That rebuilds the key from the repo-db entry as `x@l,T1,a@/b`, which
   doesn't exist. S3 reports the delete as successful anyway. Then `x@f,T0,0644` is stored, so both
   keys now exist.
3. Normal pushes and pulls are fine, because they use the repo-db.
4. Later, a 0.3.0 `init-repo` rebuild lists S3. Both keys parse, and `dbHandleObject` keeps the one
   with the newer file mtime. That's the stale link (`T1 > T0`). The real file's key goes into
   `extraKeys`, and it's rebuilt correctly because it's a file.
5. Subsequent pulls replace the file with the old symlink, and clean mode offers to delete the
   file's object.

If the replacement is newer, or the link was just retargeted, the result is milder but still wrong:
- A deleted link comes back after a rebuild.
- The stale key lands in `extraKeys` under its rebuilt name, so clean mode can never remove it.

Since old versions will always have old names, we have to accept old and new formats forever, but we
have to not trust a key rebuilt from a `FileInfo`.

- When deleting, for a link whose target contains `/`, delete both encodings in `Remove` and
  `RemoveBatch`.
- **`extraKeys`:** at `s3source.go:441,448` (`dbHandleObject`), use `*object.Key` directly instead
  of `KeyFromPath`. At line 437, it needs the key of the object that lost, so the database build
  would have to remember the actual key for each path.
...
Use [SeaweedFS](https://github.com/seaweedfs/seaweedfs) for testing or to create a local S3
API-compatible storage area for a local repository. See also [Testing](#testing)
